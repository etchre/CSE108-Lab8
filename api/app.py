...
def getStudentClasses():
    ...
    #throw an error if username not provide
    if not username:
...
def enrollInClass():
    data = request.get_json()
    #throw an error if class id is not provided
    if not data or 'username' not in data or 'classId' not in data:
        return jsonify({'error': 'Username and classId are required'}), 400
    ...
    #throw an error if the student is in the class
...
def unenrollClass():
    data = request.get_json()
    if not data or 'username' not in data or 'classId' not in data:
        return jsonify({'error': 'Username and class ID are required'}), 400
